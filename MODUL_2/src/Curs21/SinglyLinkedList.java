...
public class SinglyLinkedList<T> {
  ...
  public void insert(T newValue, int pos) {
    // Special case - inserting in place of the list head
    if (pos == 0) {
  ...
  public T remove(int pos) {
    ...
    if (pos == 0) {
      // store the value from the deleted head node
      T removedEl = listHead.value();
 
      // update the head of the list to indicate the next node
      listHead = listHead.next();
 
      // return the deleted head value
      return removedEl;
    }
